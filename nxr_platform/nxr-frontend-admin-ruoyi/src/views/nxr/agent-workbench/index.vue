...
async function initialize(){
  ...
  try{const context=await management.value.fetchContext();if(current!==initialization||!visible)return;platformManager.value=context.platformManager;const requested=Number(route.query.company);const id=context.platformManager&&Number.isSafeInteger(requested)&&requested>0?requested:context.company?.id;if(context.platformManager)await searchCompanies();if(current!==initialization||!visible)return;await selectCompany(id,false)}catch(e){if(current===initialization&&visible){error.value=e instanceof Error?e.message:'送评工作台加载失败。';loading.value=false}}finally{if(current===initialization)initializing=false}
}
...
<template><main class="nxr-workspace agent-admin-page" data-testid="agent-admin-workbench"><NxrPageHeader :title="$t('nav.submissionWorkspace')" :summary="company ? company.companyName || company.displayName : ''"><template #actions><router-link v-if="platformManager" to="/nxr/partners"><el-button data-testid="submission-open-partners">子代理管理</el-button></router-link></template></NxrPageHeader><el-card v-if="platformManager" shadow="never" class="agent-company-card"><el-form inline><el-form-item label="当前子代理企业"><el-select v-model="selectedId" filterable remote clearable :remote-method="searchCompanies" :loading="searching" :disabled="activeApi?.busy.value||loading" placeholder="搜索并选择子代理企业" style="width:360px;max-width:100%" data-testid="agent-company-select" @change="selectCompany"><el-option v-if="company&&!companies.some(item=>item.id===company?.id)" :value="company.id" :label="company.companyName||company.displayName" /><el-option v-for="item in companies" :key="item.id" :value="item.id" :label="`${item.companyName||item.displayName} · ${item.email}`" /></el-select></el-form-item></el-form></el-card><el-alert v-if="error" :title="error" type="error" :closable="false" class="agent-context-error" /><el-button v-if="error" @click="initialize">重试</el-button><p v-if="loading" class="agent-loading" role="status">加载送评工作台…</p><AgentWorkspace v-else-if="activeApi&&company" :key="`${company.id}-${generation}`" :api="activeApi" /><el-empty v-else-if="!error" :description="platformManager?'请选择子代理企业':'此后台账号尚未绑定子代理企业，请联系平台管理员。'" data-testid="agent-binding-required" /></main></template>
