...
watch(() => props.photoId, async id => {
  ...
  try {
    ...
    if (!blob.type?.startsWith('image/')) throw new Error(tx('Unable to load image, or access is denied.'))
    url.value = URL.createObjectURL(blob)
  } catch (e) { if (current === generation) error.value = e instanceof Error ? e.message : tx('Unable to load image.') }
}, { immediate: true })
onBeforeUnmount(() => { generation++; if (url.value) URL.revokeObjectURL(url.value) })
</script>
<template><figure v-if="photoId" class="agent-private-photo"><a v-if="url" :href="url" target="_blank" rel="noopener noreferrer"><img :src="url" :alt="label" /></a><span v-else>{{ error || $tx('Loading image…') }}</span><figcaption>{{ label }}</figcaption></figure></template>
