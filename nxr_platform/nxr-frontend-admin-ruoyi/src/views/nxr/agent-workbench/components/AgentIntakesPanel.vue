...
async function revealDetails() { await nextTick(); detailPanel.value?.scrollIntoView({ behavior: 'smooth', block: 'start' }); detailPanel.value?.focus({ preventScroll: true }) }
function closeDetails() { if (busy.value) return; detailGeneration++; detailLoading.value = false; mode.value = 'detail'; detail.value = null }
...
<template>
  <div class="agent-panel" data-testid="agent-intakes-panel">
    ...
    <div class="agent-list-layout">
      ...
      <section v-if="showDetails" ref="detailPanel" class="agent-detail-section" tabindex="-1" :aria-label="$tx('Intake details')" data-testid="agent-intake-details">
        <div class="agent-detail-close"><button type="button" class="text-button" :disabled="busy" data-testid="agent-close-intake-details" @click="closeDetails">{{ $tx('Close details') }}</button></div>
