...
<template>
  <div class="agent-panel" data-testid="agent-intakes-panel">
    ...
    <div v-if="createdSubmission" class="agent-success" role="status">已生成送评批次 <strong>{{ createdSubmission.batchNo }}</strong>。<router-link :to="{ path: '/nxr/submission-workbench', query: { tab: 'batches', batch: createdSubmission.batchNo, company: api.companyId } }" data-testid="agent-open-batch">前往批次订单</router-link></div>
