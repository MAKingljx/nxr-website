...
const operationOpen = computed({
  get: () => mode.value !== 'list',
  set: value => { if (!value) void closeOperation() },
})
...
<template>
  <div class="agent-panel agent-list-layout" data-testid="agent-returns-panel">
    ...
    <el-drawer v-model="operationOpen" :title="mode === 'check' ? $tx('Check returned cards') : mode === 'new' ? $tx('Create customer return') : detail?.shipment.shipmentNo || $tx('Return shipment')" size="min(1100px, 96vw)" direction="rtl" destroy-on-close :close-on-click-modal="!busy" :show-close="!busy" data-testid="agent-return-operation-drawer">
