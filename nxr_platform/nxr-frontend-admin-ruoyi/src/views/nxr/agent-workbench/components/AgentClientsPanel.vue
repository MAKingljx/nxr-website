...
const detailOpen = computed({
  get: () => editing.value || detailLoading.value || Boolean(detail.value),
  set: value => { if (!value) closeDetail() },
})
const detailTitle = computed(() => editing.value ? (editId.value ? tx('Edit customer') : tx('Add customer')) : detail.value?.client.displayName || tx('Customer records'))
...
<template>
  <div class="agent-panel" data-testid="agent-clients-panel">
    ...
    <div class="agent-list-layout">
      ...
      <el-drawer v-model="detailOpen" :title="detailTitle" size="min(920px, 96vw)" destroy-on-close :close-on-click-modal="!busy" :show-close="!busy" data-testid="agent-client-detail-drawer">
      <section v-if="editing || detailLoading || detail" ref="detailSection" class="agent-detail-pane" tabindex="-1" :aria-label="editing ? (editId ? $tx('Edit customer') : $tx('Add customer')) : $tx('Customer records')">
