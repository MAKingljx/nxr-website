<template>
  <el-drawer v-model="showSettings" :withHeader="false" :lock-scroll="false" direction="rtl" size="300px">
    <div class="setting-drawer-title">
      <h3 class="drawer-title">{{ $tx('Navigation Layout') }}</h3>
    </div>
    <div class="nav-wrap">
      <el-tooltip :content="$tx('Side navigation')" placement="bottom">
        <div class="item left" @click="handleNavType(1)" :class="{ activeItem: navType == 1 }">
          <b></b><b></b>
        </div>
      </el-tooltip>

      <el-tooltip :content="$tx('Mixed navigation')" placement="bottom">
        <div class="item mix" @click="handleNavType(2)" :class="{ activeItem: navType == 2 }">
          <b></b><b></b>
        </div>
      </el-tooltip>
      <el-tooltip :content="$tx('Top navigation')" placement="bottom">
    ...
    <div class="setting-drawer-title">
      <h3 class="drawer-title">{{ $tx('Theme Style') }}</h3>
    </div>
    <div class="setting-drawer-block-checbox">
      <div class="setting-drawer-block-checbox-item" @click="handleTheme('theme-dark')">
        <img src="@/assets/images/dark.svg" :alt="$tx('dark')" />
        <div v-if="sideTheme === 'theme-dark'" class="setting-drawer-block-checbox-selectIcon" style="display: block;">
          <i aria-hidden="true" class="anticon anticon-check">
      ...
      <div class="setting-drawer-block-checbox-item" @click="handleTheme('theme-light')">
        <img src="@/assets/images/light.svg" :alt="$tx('light')" />
    ...
    <h3 class="drawer-title">{{ $tx('Layout Options') }}</h3>

    <div class="drawer-item">
      <span>{{ $tx('Show Tabs') }}</span>
      <span class="comp-style">
    ...
    <div class="drawer-item">
      <span>{{ $tx('Tab Style') }}</span>
      <span class="comp-style">
        <el-radio-group v-model="settingsStore.tagsViewStyle" :disabled="!settingsStore.tagsView" size="small">
          <el-radio-button :label="$tx('card')">{{ $tx('Cards') }}</el-radio-button>
          <el-radio-button :label="$tx('chrome')">{{ $tx('Browser') }}</el-radio-button>
        </el-radio-group>
      </span>
    </div>

    <div class="drawer-item">
      <span>{{ $tx('Fixed Header') }}</span>
      <span class="comp-style">
    ...
    <div class="drawer-item">
      <span>{{ $tx('Dynamic Page Title') }}</span>
      <span class="comp-style">
    ...
    <el-button type="primary" plain icon="DocumentAdd" @click="saveSetting">{{ $tx('Save') }}</el-button>
    <el-button plain icon="Refresh" @click="resetSetting">{{ $tx('Reset') }}</el-button>
  </el-drawer>
...
const predefineColors = ref(["#2F6F9F", "#409EFF", "#ff4500", "#ff8c00", "#ffd700", "#90ee90", "#00ced1", "#c71585"])
...
function saveSetting() {
  proxy.$modal.loading(tx('Saving settings...'))
...
function resetSetting() {
  proxy.$cache.local.remove('tags-view-visited')
  proxy.$modal.loading(tx('Resetting settings and refreshing...'))
