...
async function load(allocate = false) {
  ...
  if (!url) throw new Error('请先选择送评订单。')
  ...
  try {
    ...
    return record
  } catch (e: any) { if (current === generation) error.value = e?.response?.data?.message || e.message || '收卡标识加载失败。'; throw e }
  finally { if (current === generation) loading.value = false }
}
async function printLabels() {
  if (printing.value) return
  const current = ++printGeneration; printing.value = true
  try { await printer.open(() => load(true), code => QRCode.toDataURL(code, { width:240,margin:3,errorCorrectionLevel:'M' })) }
  catch(e) { if(current === printGeneration) error.value = String((e as {message?:unknown})?.message || '标签生成失败。') }
...
<template><section class="card-identity-panel" data-testid="card-identity-panel"><div class="identity-heading"><div><strong>逐卡归属与收卡标签</strong><p>一张卡一个收卡编号，评级后关联证书编号。</p></div><el-button type="primary" plain :loading="printing" :disabled="loading||!endpoint" data-testid="print-card-identities" @click="printLabels">生成 / 打印收卡标签</el-button></div><el-alert v-if="error" :title="error" type="error" :closable="false"/><template v-if="value&&!compact"><el-table :data="value.items" border size="small"><el-table-column prop="itemNo" label="序号" width="65"/><el-table-column prop="cardName" label="卡片" min-width="150"/><el-table-column label="收卡编号" min-width="220"><template #default="{row}"><code>{{row.receiptCode||'待生成'}}</code></template></el-table-column><el-table-column label="原始客户" min-width="150"><template #default="{row}">{{row.ownerDisplayName}}<small v-if="row.clientReference">{{row.clientReference}}</small></template></el-table-column><el-table-column label="送评来源" min-width="160"><template #default="{row}">{{row.sourceType==='partner'?'子代理：'+row.partnerCompanyName:'客户直寄 NXR'}}</template></el-table-column><el-table-column label="回寄路线" min-width="160"><template #default="{row}">{{row.returnRoute==='via_partner'?'NXR → 子代理 → 客户':'NXR → 客户'}}</template></el-table-column><el-table-column label="证书编号" min-width="130"><template #default="{row}">{{row.certId||'评级后关联'}}</template></el-table-column></el-table></template></section></template>
