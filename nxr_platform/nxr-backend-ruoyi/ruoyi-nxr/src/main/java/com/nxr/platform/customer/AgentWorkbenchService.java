...
public class AgentWorkbenchService {
    ...
    public record IntakeCardRequest(String cardName, String languageCode, String notes) { }
    ...
    public record Card(long id, long intakeId, String intakeNo, long clientId, String clientName,
        String inventoryCode, String cardName, String languageCode, String notes, String statusCode,
    ...
    private static final String CARD_SELECT = """
        SELECT c.id, c.intake_id, i.intake_no, i.client_id, cl.display_name AS client_name,
               c.inventory_code, c.card_name, c.language_code, c.notes, c.status_code, c.condition_note,
    ...
    public IntakeDetail createIntake(long owner, IntakeRequest r) {
        ...
        for (IntakeCardRequest c : r.cards()) {
            ...
            jdbc.sql("""
                INSERT INTO agent_card (merchant_customer_id,intake_id,inventory_code,card_name,language_code,notes)
                VALUES (:owner,:intake,:code,:name,:language,:notes)
                """).params(params("owner",owner,"intake",id,"code",number("AC"),"name",required(c.cardName(),"Card name",255),
                    "language",required(c.languageCode(),"Language",32).toUpperCase(Locale.ROOT),"notes",optional(c.notes(),2000))).update();
        }
    ...
    public Submission submit(long owner,SubmissionRequest r) {
        ...
        for(long id:ids) {
            ...
            List<CustomerPortalService.OrderItemRequest> items=cards.stream().map(c -> new CustomerPortalService.OrderItemRequest(
                c.cardName(),null,null,null,c.languageCode(),BigDecimal.ZERO,c.notes())).toList();
