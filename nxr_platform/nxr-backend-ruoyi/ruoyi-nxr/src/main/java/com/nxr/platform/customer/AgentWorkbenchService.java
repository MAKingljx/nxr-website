...
public class AgentWorkbenchService {
    ...
    public Page<Card> cards(long owner,int page,int size,Long clientId,Long intakeId,String status,String query) {
        ...
        if(optional(query,255)!=null) { where+=" AND (LOWER(c.inventory_code) LIKE :q OR LOWER(c.official_card_number) LIKE :q OR LOWER(c.card_name) LIKE :q OR LOWER(gs.cert_id) LIKE :q OR LOWER(i.intake_no) LIKE :q OR LOWER(cl.display_name) LIKE :q)"; p.put("q",search(query)); }
