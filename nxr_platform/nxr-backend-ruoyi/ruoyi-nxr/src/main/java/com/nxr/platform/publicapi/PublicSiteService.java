...
public class PublicSiteService {
    ...
    private static final java.util.Map<String, String> CARD_CATEGORY_LABELS = java.util.Map.of(
    ...
    public WaitlistCountResponse loadWaitlistCount() {
        return new WaitlistCountResponse(countWaitlistEmails());
    }

    public WaitlistSignupResponse joinWaitlist(WaitlistSignupRequest request) {
        String email = normalizeEmail(request.email());
        boolean alreadyJoined = jdbcClient.sql("SELECT COUNT(*) FROM waitlist_email WHERE email = :email")
            .param("email", email)
            .query(Integer.class)
            .single() > 0;
        ...
        if (!alreadyJoined) {
            jdbcClient.sql(
                    """
                    INSERT INTO waitlist_email (email, source_code, status_code)
                    VALUES (:email, 'web', 'pending')
                    """
                )
                .param("email", email)
                .update();
    ...
    private int countWaitlistEmails() {
        return jdbcClient.sql("SELECT COUNT(*) FROM waitlist_email")
            .query(Integer.class)
            .single();
    }

    private String cardCategoryLabel(String value) {
        String normalized = value == null || value.isBlank() ? "trading_card" : value.trim();
