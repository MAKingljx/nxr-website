...
def api(path, payload=None, token=None, admin=False, method=None, blocked=None, scope=None):
    headers = {'Content-Type': 'application/json'}
    if scope is not None:
        headers['X-NXR-Agent-Id'] = str(scope)
    if token:
...
def upload(path, token, expected=200):
    ...
    body = (f'--{boundary}\r\nContent-Disposition: form-data; name="file"; filename="fixture.png"\r\nContent-Type: image/png\r\n\r\n'.encode()
            + png() + f'\r\n--{boundary}--\r\n'.encode())
    req = urllib.request.Request(BASE + path, data=body, headers={'Content-Type': f'multipart/form-data; boundary={boundary}', 'Authorization': 'Bearer ' + token})
    try:
...
def main():
    ...
    prefix = '/api/admin/agent'
    operators = []
    operator_password = 'Aq9!' + secrets.token_hex(7)  # RuoYi accepts 5-20 characters.
    for index, account in enumerate([agent, rival]):
        username = f'agent_qa_{index}_{suffix}'
        admin('/system/user', {'userName': username, 'nickName': f'QA agent operator {index}', 'password': operator_password,
                              'status': '0', 'roleIds': [], 'postIds': []})
        users = admin('/system/user/list?userName=' + username)['rows']
        user_id = next(x['userId'] for x in users if x['userName'] == username)
        unbound = api('/login', {'username': username, 'password': operator_password})
        api(prefix + '/context', token=unbound['token'], admin=True, blocked={403})
        admin(prefix + f'/operators/{user_id}', {'merchantCustomerId': account['customer']['id'], 'active': True}, method='PUT')
        login = api('/login', {'username': username, 'password': operator_password})
        operators.append({'token': login['token'], 'userId': user_id, 'userName': username, 'password': operator_password})
    operator, other_operator = operators
    customer = lambda path, payload=None, **kw: api(path, payload, operator['token'], True, **kw)
    other = lambda path, payload=None, **kw: api(path, payload, other_operator['token'], True, **kw)
    portal = lambda path, payload=None, **kw: api(path, payload, agent['token'], **kw)
    check('agent operator is scoped to its bound company', customer(prefix + '/context')['company']['id'] == agent['customer']['id'])
    check('agent operator cannot manage company bindings', not customer(prefix + '/context')['platformManager'])
    check('company selector is limited to the bound company', customer(prefix + '/companies')['total'] == 1)
    check('platform account can manage company bindings', admin(prefix + '/context')['platformManager'])
    check('platform explicit company scope is supported', admin(prefix + '/clients', scope=agent['customer']['id'])['total'] == 0)
    customer(prefix + '/clients', scope=rival['customer']['id'], blocked={403})
    customer(prefix + '/operators', blocked={403})
    customer(prefix + f"/operators/{other_operator['userId']}", {'merchantCustomerId': agent['customer']['id'], 'active': True}, method='PUT', blocked={403})
    for forbidden in ['/api/admin/customers', '/api/admin/orders', '/api/admin/dashboard', '/system/user/list']:
        customer(forbidden, blocked={403})
    info = customer('/getInfo')
    check('agent role has no global business permission', set(info['permissions']) == {'nxr:agent:workbench'})
    routes = customer('/getRouters')
    check('agent menu includes its workbench', 'nxr/agent-workbench/index' in json.dumps(routes))
    def route_paths(nodes, parent=''):
        for node in nodes:
            path = node.get('path', '')
    ...
    api('/api/customer/agent/clients', token=agent['token'], blocked={401, 403, 404})
    check('no payment provider is enabled', api('/api/customer/payment-options?currency=USD') == [])
    check('mail delivery remains disabled', not portal('/api/customer/account/email-status')['delivery']['available'])
    ...
    bob = customer(prefix + '/clients', key({**client_base, 'reference': 'BOB', 'displayName': 'QA Bob', 'contactName': 'QA Bob'}))
    ...
    address = customer(prefix + '/addresses', {'label': 'Agent warehouse', 'contactName': 'QA Agent', 'contactPhone': '15550100999', 'addressLine1': '10 Agent Road', 'addressLine2': '', 'city': 'Test City', 'region': 'CA', 'postalCode': '90002', 'country': 'US', 'defaultAddress': True})
    ...
    uploaded = upload(prefix + f'/cards/{all_cards[0]["id"]}/photos?side=front', operator['token'])
    photo_id = uploaded['frontPhotoId']; check('agent intake photo is attached to its card', photo_id is not None)
    portal(f'/api/customer/order-photos/{photo_id}', method='DELETE', blocked={409})
    other(prefix + f'/order-photos/{photo_id}', blocked={404})
    upload(prefix + f'/cards/{all_cards[0]["id"]}/photos?side=back', other_operator['token'], 404)
    ...
    check('stale quote leaves no NXR batch or stock linkage', customer(prefix + f'/intakes/{aid}')['intake']['orderId'] is None and customer(prefix + '/merchant/batches')['total'] == 0)
    ...
    batch = customer(prefix + '/merchant/batches/' + batch_no)
    ...
    photo_order = customer(prefix + '/orders/' + next(x['orderNo'] for x in stock if x['id'] == all_cards[0]['id']))
    ...
    customer(prefix + f'/clients/{bob["id"]}', {**client_base, 'reference': 'BOB', 'displayName': 'QA Bob', 'contactName': 'QA Bob', 'active': False}, method='PUT')
    ...
    check('NXR delivery state was not overwritten by client final-mile events', customer(prefix + '/merchant/batches/' + batch_no)['statusCode'] == 'delivered')
    # Bindings and company status must invalidate an already-issued backend session immediately.
    admin(prefix + f"/operators/{operator['userId']}", {'merchantCustomerId': agent['customer']['id'], 'active': False}, method='PUT')
    customer(prefix + '/clients', blocked={403})
    admin(prefix + f"/operators/{operator['userId']}", {'merchantCustomerId': agent['customer']['id'], 'active': True}, method='PUT')
    check('reactivating operator preserves all client history', customer(prefix + '/clients')['total'] == 2)
    ...
    customer(prefix + '/clients', blocked={403})
    admin(f"/api/admin/customers/{agent['customer']['id']}/status", {'active': True}, method='PUT')
    check('company reactivation retains submitted batches', customer(prefix + '/merchant/batches')['total'] == 1)
    admin(prefix + '/operators/1', {'merchantCustomerId': agent['customer']['id'], 'active': True}, method='PUT', blocked={400, 403, 409})
    admin(prefix + f"/operators/{other_operator['userId']}", {'merchantCustomerId': collector['customer']['id'], 'active': True}, method='PUT', blocked={400, 403, 409})
    check('invalid operator binding does not change its prior company', other(prefix + '/context')['company']['id'] == rival['customer']['id'])
    session_file = Path('/tmp') / f'{database}-session.json'
    session_file.write_text(json.dumps({'agent': agent, 'rival': rival, 'collector': collector, 'agentOperator': operator, 'rivalOperator': other_operator, 'adminToken': admin_token, 'clientIds': [alice['id'], bob['id']], 'intakeIds': [aid, bid], 'batchNo': batch_no, 'shipmentId': sent['id']}))
