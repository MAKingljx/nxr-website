...
files += [p for p in sorted((platform / 'nxr-sql/ruoyi').glob('*.sql')) if int(p.name[:2]) in [1, 2, 3, 4, 5, *range(10, 26)]]
if not all(any(p.name.startswith(prefix) for p in files) for prefix in ('23_', '24_', '25_')):
    raise SystemExit('Agent workbench migrations 23 through 25 must exist first.')
for file in files:
