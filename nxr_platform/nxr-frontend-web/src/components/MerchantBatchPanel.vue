...
onMounted(async () => {
  await load()
  if (typeof route.query.batch === 'string' && route.query.batch.length <= 48) await select(route.query.batch)
})
watch(() => route.query.batch, value => {
  if (typeof value === 'string' && value.length <= 48 && value !== selected.value?.batchNo) void select(value)
})
