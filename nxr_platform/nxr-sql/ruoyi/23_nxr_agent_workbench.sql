-- Agent-owned customer intake and final-mile parcels. No account, wallet or live data is seeded.
-- Run after 22_nxr_customer_order_photos.sql; safe to rerun.
DROP PROCEDURE IF EXISTS nxr_agent_photo_column;
DELIMITER $$
CREATE PROCEDURE nxr_agent_photo_column()
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = DATABASE()
                   AND table_name = 'customer_order_photo' AND column_name = 'preserved_for_agent') THEN
        ALTER TABLE customer_order_photo ADD COLUMN preserved_for_agent TINYINT NOT NULL DEFAULT 0;
    END IF;
END$$
DELIMITER ;
CALL nxr_agent_photo_column();
DROP PROCEDURE nxr_agent_photo_column;
